...
class TestMD5Tracker < Test::Unit::TestCase
  ...
  def test_identical_files_match
    ...
    @file1.close
    file1_md5 = Metricks::MD5Tracker.track(@file1.path)
    
    @file2.puts("Hello World")
    @file2.close
    file2_md5 = Metricks::MD5Tracker.track(@file2.path)
  ...
  def test_file_changed
    ...
    @file1.close
    assert Metricks::MD5Tracker.file_changed?(@file1.path)
  end
  ...
  def test_file_changed_if_not_tracking
    ...
    assert Metricks::MD5Tracker.file_changed?(@file1.path)
    assert File.exist?(Metricks::MD5Tracker.md5_file(@file1.path))
  end
end
